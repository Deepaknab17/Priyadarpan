...
def generate_session_recommendations(user,mood):
    ...
    session=MoodSession.objects.create(tenant=tenant,user=user,mood=mood)
    ...
    for s in songs:
        if s.id not in seen:
            ...
            seen.add(s.id)
    songs=unique_songs
    interactions = UserSongInteraction.objects.filter(user=user,tenant=tenant,mood=mood)
    ...
    ranked = sorted(indexed_songs,key=lambda pair:rank_value(pair[0],pair[1]),reverse=True)
    songs = [s for _, s in ranked][:10]
    recs = [
        SessionRecommendation(session=session,song=song,rank=rank)
